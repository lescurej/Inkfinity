...
export const useCanvasStore = create<CanvasState>((set, get) => ({
  ...
  setViewport: (v: Partial<Viewport>) =>
    set((state: CanvasState) => ({ viewport: { ...state.viewport, ...v } })),
  ...
  addSegmentToChunk: (seg: DrawingSegment) => {
    const chunkKey = `${Math.floor(seg.x2 / CHUNK_SIZE)},${Math.floor(
      seg.y2 / CHUNK_SIZE
    )}`;
...
export const CanvasProvider: React.FC<CanvasProviderProps> = ({ children }) => {
  ...
  const addSegmentToChunk = useCallback(
    (seg: DrawingSegment) => {
    ...
    (e: React.WheelEvent<HTMLDivElement>) => {
      ...
      if (e.ctrlKey || e.metaKey || e.shiftKey) {
        ...
        const newScale = Math.max(
          0.1,
          Math.min(5, viewport.scale * zoomFactor)
        );
        setViewport((_) => ({
          scale: newScale,
